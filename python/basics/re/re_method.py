...
# (我们将在下一个部分看到, 事实上匹配对象在处理分组问题时非常有用), 但是匹配对象还包含了更多关于匹配子串的信息
...
contact_info = 'Doe, John: 555-1212-white'
match = re.search(r'\w+, \w+: \S+', contact_info)
if match:
    print match.group(0)
...
match = re.search(r'(?P<last>\w+), (?P<first>\w+): (?P<phone>\S+)', contact_info)
if match:
    match.group('last')
    match.group('first')
    match.group('phone')
