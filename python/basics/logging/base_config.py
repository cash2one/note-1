...
# 以ERROR级别记录日志消息, 异常跟踪信息将被自动添加到日志消息里. logging.exception通过用在异常处理块中
...
# logging.getLevelName(level)  # 获取日志级别对应的名称
...
logging.handlers.RotatingFileHandler 类似于上面的FileHandler,当文件达到一定大小之后,它会自动将当前日志文件改名,然后创建一个新的同名日志文件继续输出
